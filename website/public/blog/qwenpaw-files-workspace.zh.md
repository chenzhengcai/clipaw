...
这个区分很重要。选择 Project Directory 会同时更新 Files 当前展示的项目根目录，以及当前 Chat 在普通任务中继承的有效项目目录；Agent Configuration Directory 则始终用于查看 Agent 自己的文件。

Project Directory 还支持按对话继承和切换。Agent 可以设置一个默认目录；单个 Chat 也可以选择自己的目录，并把这个选择保存在当前对话中。Files 会使用当前对话实际生效的目录。在普通 Chat 任务中，如果没有另外指定路径或 Shell 的工作目录，Agent 的文件读写与搜索、Shell 和代码分析会把这个目录作为默认根目录；Files 中的 Git 操作也使用这个目录。清除对话中的选择后，它会重新继承 Agent 的默认目录。任务模式和 fork 工作区需要隔离运行时，可以使用优先级更高的专用目录。
...
这些日记不是一份难以阅读的完整聊天备份。它们会保留真正有用的信息；由 ReMe Light 自动记忆流程生成的日记还会在文件元数据中记录来源会话标识，方便核对它来自哪次对话。
...
关系图让这些内容不再是互相孤立的文件。节点对应已索引的 Daily、Knowledge Base 文件或分类入口，连线来自文件中的 wiki 链接。你可以直接打开图中已索引的文件；如果知识文件链接了相关日记或方法，也可以沿着连线继续查看。
...
Files 处理的是本地文件，因此文件接口会限制访问范围，拒绝路径穿越和逃出所选目录的符号链接，并在上传重名文件时要求明确选择处理方式。无法确认位于当前 Project Directory 或 Agent Configuration Directory 中的历史附件会以只读方式打开，而不会假装仍可编辑。保存 Workspace 中的文本文件时，Files 会核对打开文件时记录的版本；如果磁盘上的文件已经变化，就会拒绝用旧内容覆盖它。通过检查的内容会以原子方式写入。

当前 Files 更适合常规规模的工作区。目录内容会分页显示，但文件树还没有搜索和长列表虚拟化；文本会由服务端分块读取，不过当前界面会等所有分块读取完成并拼成完整内容后再开始预览或编辑。因此，打开超大文本文件或包含大量同级文件的目录时，可能需要等待更久。

更重要的是，记忆和配置并没有藏在一个无法检查的黑盒里。它们以文件形式保存在 Agent Configuration Directory 中，并通过对应的 Files 入口打开、阅读和修改。
