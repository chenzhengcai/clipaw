...
1. **★首要原则：不直接改上游源码文件**。定制尽量以"新增独立文件"或"集中修改 fork 专属文件"（`console/src/themes/purple/overrides.css`、`themes/`、本目录）的方式落地，最小化与上游的冲突面。集成点无法绕过时：上游文件内的改动压到最小（几行 import/挂载），主体逻辑放 fork 专属新文件，并在此留档冲突检查点
2. **每次个性化修改都在此目录留档**，记录改了什么、为什么改、怎么和上游合并
3. **上游更新后**，按文档中的"冲突分析"章节逐项检查
4. **定期审查**已有修改是否还有必要保留（上游可能已原生支持）
5. **文档风格**：结构化表格 > 段落叙述，代码 diff > 文字描述，ASCII 图 > 自然语言流程
