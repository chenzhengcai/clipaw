...
| 2026-07-02 (App.tsx 集成) | App.tsx 新增 `loadClientConfig()` 调用，恢复语音设置和 Agent 选择 | App.tsx || 2026-09-11 (--app-* token 接管) | **适配上游 #7487/#7502 语义 token 体系**：上游已把全部 module.less 迁到 `--app-*` 变量（styles/tokens.css 单一事实源），旧 `--color-*` 覆盖退化为仅 overrides.css 内部引用。tokens.css 新增 `--app-*` 覆盖层（accent 家族 9 变量 × 亮暗 + 表面微紫调 + 亮色导航选中紫黑 tint），一处接管 142+ 个 `var(--app-accent*)` 消费点，含 OS 桌面壳（os/useOsStyles.ts 的 ACCENT 即 `var(--app-accent)`）与暗色导航选中；删除 0 消费者的死 token（--space-*/--radius-*/--shadow-primary/--transition-fast/--font-family） | tokens.css |
| 2026-09-11 (硬编码橙残留补丁) | **overrides.css 附录 A**：--app-* 接管后仍绕过 token 的 8 处硬编码橙逐个补紫——layouts/index appBrand:hover、sidebarSettingsPanel menuItem/choiceItem 触发器（亮/暗）、sidebarSessionList historyAction:focus-visible outline + 暗色 --sidebar-create-hover-* 局部变量、PawApps appCard（`:has(appIframe)` 锚点避免与 AppCenter 的同名 index-module__appCard 撞车）、MagazineStackViewer .active、CreateHarvestModal templateCardActive、Agents 暗色 .ant-btn-text:hover | overrides.css (+~104 行) |
| 2026-09-11 (结构对账说明) | **本文档 §3.1 改动清单已过时**：Sidebar 自定义导航（SectionHeader）与 PluginManager 主题插件已由 `91afbf02`/`557a1a71` 有意回归上游方案（主题差异收敛至 themes/ + SidebarSettingsPanel 切换），勿按旧清单"恢复"。生效架构见 themes/purple/tokens.css 头部注释与 clipaw2x-migration-audit.md | 文档 |
