...
- **全局背景**:向 `body` 注入一个 `position: fixed; z-index: -1` 的媒体层,并给 `html/body` 加 `qwp-bg-global-on` class,配合注入的 CSS 将 layout/header/sider/page-content 背景置透明。header/sider 及聊天消息列表等规则统一使用 `html+body` 双前缀(0,3,2)以保证在暗色模式与紫色主题的 `!important` 规则(最高 0,3,1,如 `html[data-theme="purple"].dark-mode .qwenpaw-layout-sider`)下依然生效,并覆盖 `qwenpaw-layout-sider-dark`、`__siderDark__`、`chat-anywhere-message-list` 等暗色专属类名。
